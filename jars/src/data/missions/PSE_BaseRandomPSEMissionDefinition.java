...
/*
from tiandong with permission, credit to MesoTroniK and Histidine. I have commented out the original station generation
capability for simplicity, as well as other things relating to tiandong specific fleet generation and autofitting
 */
public class PSE_BaseRandomPSEMissionDefinition implements MissionDefinitionPlugin {
    public static Logger log = Global.getLogger(PSE_BaseRandomPSEMissionDefinition.class);
    ...
    public void defineMission(MissionDefinitionAPI api)
    {
        ...
        */
        ...
        if (Math.random() < STATION_CHANCE)
        {
            ...
            if (stationSide == 0)
            {
                stationPlayer = THIMissionStationDefs.getStationForFaction(player.getId(), size * difficulty * 2);
            }
            else if (stationSide == 1)
            {
                stationEnemy = THIMissionStationDefs.getStationForFaction(enemy.getId(), size * 2);
            }
        }
    ...
    protected int generateFleet(FactionAPI faction, MissionDefinitionAPI api, FleetSide side, int fp/*, AvailableStation station*/)
    {
        ...
        if (station != null)
        {
            ...
            float removeMult = THIMissionStationDefs.getModuleRemovalChanceForFaction(factionId);
            if (removeMult > 0)
            {
                THIMissionStationDefs.removeModulesFromStation(stationMember, removeMult);
            }
            fp -= station.fpValue;
        }
         */
        ...
        if (randomMode)
        {
            ...
            dfip.mode = FactionAPI.ShipPickMode.PRIORITY_THEN_ALL;
